...
let getItemAt = (x, y, grid) =>
  ...
  |> (
    fun
    | None =>
      raise(Failure({j|Failed to find element at ($x, $y) \n $grid|j}))
    | Some(x) => x
  );
...
let getItemsDirection = (~fx, ~fy, ~switchOn, ~bailOn, x, y, grid) => {
  ...
  ((x, y) |> switchOn == bailOn ? [] : go(fx(x), fy(y), grid))
  |> List.toArray;
};
...
let getLen = side => getGridDimensions >> side >> sub1;

// TODO: review later - good use of currying or no?
let getItemsAbove =
  getItemsDirection(~fx=id, ~fy=sub1, ~switchOn=Tuple.second, ~bailOn=0);
...
let getItemsLeft =
  getItemsDirection(~fx=sub1, ~fy=id, ~switchOn=Tuple.first, ~bailOn=0);
...
let calculateScenicScore = (x, y, grid) => {
  ...
  grid
  |> getAllRelevantItems(x, y)
  |> Array.map(arr => arr |> Array.toList |> doTheThing |> List.length)
  |> Array.Int.product;
};
